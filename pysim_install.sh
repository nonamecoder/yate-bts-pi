...
sudo apt install -y --no-install-recommends pcscd libpcsclite-dev python3 python3-dev python3-pip python3-full
git clone https://github.com/osmocom/pysim.git
cd ./pysim
if [[ -z "$VIRTUAL_ENV" ]]; then
    echo "No Python virtual environment is active."
    python3 -m venv $USER_HOME/.pyvenv
    echo "source $USER_HOME/.pyvenv/bin/activate" >> $USER_HOME/.bashrc
    source $USER_HOME/.pyvenv/bin/activate
else
